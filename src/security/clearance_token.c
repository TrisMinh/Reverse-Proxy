...
static void build_client_hash(const char *ip, const char *ua, const char *secret, char *out_hex, size_t outsz)
{
    ...
    HMAC(EVP_sha256(), secret, (int)strlen(secret),
         (unsigned char*)buf, (unsigned int)strlen(buf),
         mac, &mac_len);
...
char *generate_clearance_token(const char *ip, const char *ua, const char *secret) {
    ...
    HMAC(EVP_sha256(), secret, (int)strlen(secret),
         (unsigned char*)src, (unsigned int)strlen(src),
         sig, &sig_len);
...
int verify_clearance_token(const char *token, const char *ip,
                           const char *ua, const char *secret, int ttl_sec)
{
    if (!token || !secret) return 0;

    unsigned ts = 0;
    char chash[65], sig[65];
    if (sscanf(token, "%u|%64[^|]|%64s", &ts, chash, sig) != 3)
        return 0;

    unsigned now = (unsigned)time(NULL);
    if (now < ts || now - ts > (unsigned)ttl_sec)
        return 0; // hết hạn

    char expected_chash[65];
    build_client_hash(ip, ua, secret, expected_chash, sizeof(expected_chash));
    if (!hex_equals(expected_chash, chash))
        return 0;
